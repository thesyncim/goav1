...
// (tracked2D, WithGeo trusted scan-hot, WithGeo untrusted); the D3-b arm64
// kernel joins the same lockstep as a fourth variant so it can be diffed
// bit-for-bit. It extends (does not duplicate) the per-read entropy kernel
// differential in internal/av1/entropy/reader_arch_diff_test.go: that test
// pins single symbol reads; this one pins whole-TXB sequences with chained
// CDF adaptation, refill boundaries, and past-end ecLotsBits accounting.
package tile
