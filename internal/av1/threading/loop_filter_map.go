...
func FrameWorkLoopFilterBlockFromVisit(block tile.BlockVisit) (FrameWorkLoopFilterBlock, error) {
	...
	return FrameWorkLoopFilterBlock{
		...
		MIColEnd:  uint16(block.MIColEnd),
		MIRowEnd:  uint16(block.MIRowEnd),
		X4:        block.X4,
		Y4:        block.Y4,
		Size:      block.Size,
...
func (m FrameWorkLoopFilterMap) Reset() error {
	...
	clear(m.Records[:int(m.Stride)*int(m.Rows)])
	return nil
}
...
func (m FrameWorkLoopFilterMap) RecordAt(miCol uint32, miRow uint32) (FrameWorkLoopFilterBlockRecord, bool, error) {
	...
	record := m.Records[int(miRow)*int(m.Stride)+int(miCol)]
	if !record.Valid {
...
func (m FrameWorkLoopFilterMap) ForEachBlock(visit func(FrameWorkLoopFilterBlockRecord) error) error {
	return m.ForEachBlockInGrid(int(m.Stride), int(m.Rows), visit)
}
...
func (m FrameWorkLoopFilterMap) validateGrid(cols int, rows int) error {
	...
	if cols <= 0 || rows <= 0 || cols > int(m.Stride) || rows > int(m.Rows) {
		return ErrInvalidBatch
	}
	return nil
}
...
func frameWorkLoopFilterRecordCovers(record FrameWorkLoopFilterBlockRecord, col int, row int, cols int, rows int) bool {
	...
	return col >= miCol && col < miColEnd &&
		row >= miRow && row < miRowEnd
}
