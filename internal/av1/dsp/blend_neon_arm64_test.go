...
// TestBlendA64MaskNEONMatchesPureGo runs the NEON inner loop and the pure-Go
...
// the NEON wrapper routes to the reference).
func TestBlendA64MaskNEONMatchesPureGo(t *testing.T) {
			...
			for _, subY := range []bool{false, true} {
				for _, width := range []int{8, 16, 32, 64} {
					for _, height := range []int{4, 8, 16} {
						...
						gotOK := blendA64MaskNEON(blendA64MaskArgs{
						...
						if gotOK != wantOK {
							t.Fatalf("bd=%d subX=%v subY=%v w=%d h=%d verdict: neon=%v ref=%v", bitDepth, subX, subY, width, height, gotOK, wantOK)
						}
						for i := range got {
							if got[i] != want[i] {
								t.Fatalf("bd=%d subX=%v subY=%v w=%d h=%d sample %d: neon=%d ref=%d", bitDepth, subX, subY, width, height, i, got[i], want[i])
							}
						}
					}
				}
...
// TestBlendA64MaskNEONRejectsOutOfRange confirms the NEON path reports the same
// invalid verdict as the reference when a sample or mask weight is out of
// range, for both vectorised layouts.
func TestBlendA64MaskNEONRejectsOutOfRange(t *testing.T) {
	if !cpu.Detected.NEON {
		t.Skip("NEON not detected")
	}
	...
	for _, subXY := range []bool{false, true} {
		...
		newArgs := func() blendA64MaskArgs {
			...
			return blendA64MaskArgs{
				...
				src1: src1, src1Stride: width,
				mask: mask, maskStride: mw,
				width: width, height: height, max: 0xff,
		...
		t.Run(layoutName(subXY)+"-sample", func(t *testing.T) {
			...
			if blendA64MaskNEON(a) {
				t.Fatal("NEON accepted out-of-range sample")
			}
		})
		t.Run(layoutName(subXY)+"-mask", func(t *testing.T) {
			...
			if blendA64MaskNEON(a) {
				t.Fatal("NEON accepted out-of-range mask weight")
			}
		})
	}
}
