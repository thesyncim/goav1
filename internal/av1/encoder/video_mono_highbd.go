...
func (e *HighBitDepthMonochromeVideoEncoder) UpdateRateControlConfig(rc RateControlConfig) error {
	if e == nil {
		return fmt.Errorf("encoder: nil high-bit-depth monochrome video encoder")
	}
	if !e.rcEnabled {
...
func (e *HighBitDepthMonochromeVideoEncoder) encodeKeyTilePayloads(seq SequenceHeader, src SourceFrameMono16, recon *SourceFrameMono16, qIndex uint8, header *IntraFrameHeaderParams) ([]TilePayload, error) {
	...
	for t := 0; t < nTiles; t++ {
		...
		data, err := pc.encodeHighBitDepthMonochromeKeyframeTileWithOptions(src, recon, qIndex, c0, c1, header.Prefix.AllowScreenContentTools)
		if err != nil {
...
func (e *HighBitDepthMonochromeVideoEncoder) configureTilePayloads(seq SequenceHeader, width, height int, tiles *TileInfo) (int, error) {
...
func (e *HighBitDepthMonochromeVideoEncoder) ensureTilePayloads(nTiles int) []TilePayload {
	if nTiles < 1 {
		nTiles = 1
	}
...
func (e *HighBitDepthMonochromeVideoEncoder) assembleKeyTU(seq SequenceHeader, header IntraFrameHeaderParams, payloads []TilePayload) ([]byte, error) {
...
func (e *HighBitDepthMonochromeVideoEncoder) resetRCTemporalState() {
...
func (e *HighBitDepthMonochromeVideoEncoder) rcUpdate(frameBits int, temporalID uint8) {
	if !e.rcEnabled {
		return
	}
