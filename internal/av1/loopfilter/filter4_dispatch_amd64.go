...
// init binds the narrow deblocking kernel dispatch slots on amd64. When the CPU
// advertises AVX2 (and the cpu package reports it), the 8-bit and 10/12-bit
// slots route through the hand-written AVX2 asm, which processes sixteen
// horizontal edge positions per 256-bit vector and falls back to the pure-Go
// reference for vertical edges and short tails. Otherwise the slots keep the
...
func init() {
	...
	if cpu.Detected.AVX2 {
		...
		filter4Edge16Impl = filter4Edge16AVX2
		return
	}
