...
// inverseADST16Core is the natural-order, single-buffer transliteration of
// inverseADST16To (with in==out==c, inStride==outStride==stride, outOffset==0).
// It is the source the itxgen NEON kernel generator (tools/itxgen) parses: its
// c[K*stride] loads/stores and clipRange/roundShift/clipInt32 idioms map
// directly onto the four-lane int32 NEON butterfly. It is verified
// bit-for-bit identical to inverseADST16 by TestInverseADST16CoreMatchesTo.
// Keep it a mechanical mirror of inverseADST16To's non-flip path.
func inverseADST16Core(c []int32, stride int, min int32, max int32) {
	in0 := int64(c[0*stride])
	in1 := int64(c[1*stride])
	in2 := int64(c[2*stride])
	in3 := int64(c[3*stride])
	...
	in6 := int64(c[6*stride])
	in7 := int64(c[7*stride])
	in8 := int64(c[8*stride])
	in9 := int64(c[9*stride])
	in10 := int64(c[10*stride])
	in11 := int64(c[11*stride])
	in12 := int64(c[12*stride])
	in13 := int64(c[13*stride])
	in14 := int64(c[14*stride])
	...
	c[0*stride] = clipRange(t0+t2, min, max)
	c[15*stride] = clipInt32(-int64(clipRange(t1+t3, min, max)))
	t2a = int64(clipRange(t0-t2, min, max))
	t3a = int64(clipRange(t1-t3, min, max))
	c[3*stride] = clipInt32(-int64(clipRange(t4a+t6a, min, max)))
	c[12*stride] = clipRange(t5a+t7a, min, max)
	t6 = int64(clipRange(t4a-t6a, min, max))
	t7 = int64(clipRange(t5a-t7a, min, max))
	c[1*stride] = clipInt32(-int64(clipRange(t8a+t10a, min, max)))
	c[14*stride] = clipRange(t9a+t11a, min, max)
	t10 = int64(clipRange(t8a-t10a, min, max))
	t11 = int64(clipRange(t9a-t11a, min, max))
	c[2*stride] = clipRange(t12+t14, min, max)
	c[13*stride] = clipInt32(-int64(clipRange(t13+t15, min, max)))
	t14a = int64(clipRange(t12-t14, min, max))
	t15a = int64(clipRange(t13-t15, min, max))

	c[7*stride] = clipInt32(-roundShift((t2a+t3a)*181, 8))
	c[8*stride] = clipInt32(roundShift((t2a-t3a)*181, 8))
	c[4*stride] = clipInt32(roundShift((t6+t7)*181, 8))
	c[11*stride] = clipInt32(-roundShift((t6-t7)*181, 8))
	c[6*stride] = clipInt32(roundShift((t10+t11)*181, 8))
	c[9*stride] = clipInt32(-roundShift((t10-t11)*181, 8))
	c[5*stride] = clipInt32(-roundShift((t14a+t15a)*181, 8))
	c[10*stride] = clipInt32(roundShift((t14a-t15a)*181, 8))
}
