...
// test) for DCT4, DCT8, DCT16, ADST4 and ADST8. Only DCT8 and DCT16 are bound
// here: their butterflies are large enough that two-row vectorisation beats
// the scalar pure-Go kernels on this host, whereas the DCT4/ADST4/ADST8 NEON
// bodies do not amortise the assembly-call and slice-to-pointer overhead over
// their smaller / clamp-heavy work and measured slower than pure-Go (see the
// Row2 benchmarks). Those three stay on pure-Go. The unbound kernels remain
// available for re-evaluation if the call overhead or kernel shape changes.
func init() {
