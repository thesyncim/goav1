...
const (
	ecMinProb   = 4
	ecProbShift = 6
	// ecWindow is the coding-window width in bits. dav1d runs the identical
	// range decoder over a 64-bit window (src/msac.h ec_win = size_t) so each
	// refill buffers ~6 bytes instead of 2-3; only WHEN bytes are buffered
	// changes, never the symbol arithmetic, which reads and renormalizes the
	// top 16 bits exactly as with the 32-bit window.
	ecWindow   = 64
	ecLotsBits = 0x4000

	// readerInitTellOffs is the od_ec tell origin. libaom writes it as
	// 10 - (OD_EC_WINDOW_SIZE - 8) (entdec.c od_ec_dec_init), but that formula
	// bakes in its fixed 32-bit window: pos*8 - cnt is window-width invariant
	// (every buffered byte advances both by 8), so widening the window must
	// NOT rescale this constant or every BitsRead consumer (tile trailing-bit
	// checks) would shift by ecWindow-32.
	readerInitTellOffs = 10 - (32 - 8)
...
// refillState extends dif with the AV1 range decoder's next bytes. It is the
// window-width port of dav1d ctx_refill (src/msac.c) / arm64 msac.S L(refill):
// with at least 8 buffered bytes remaining, one big-endian 8-byte load fills
// the whole 64-bit window (6-7 bytes consumed per refill instead of 2-3),
// keeping goav1's libaom XOR-into-ones window convention (entdec.c
// od_ec_dec_refill) so the inserted bytes land bit-identically to the byte
// loop. Short buffer tails fall back to the byte loop.
//
//go:nosplit
func refillState(src []byte, pos int, dif uint64, cnt int32, tellOffs int32) (int, uint64, int32, int32) {
	// shift is the bit position of the next byte's top bit, exactly as in the
	// byte loop below; cnt < 0 at every hot call site, so shift is in
	// [ecWindow-24, ecWindow-9] and n below is 6 or 7 (never exhausting the
	// >= 8 remaining bytes, so no end-of-buffer accounting on this path).
	shift := int32(ecWindow) - 9 - (cnt + 15)
	if uint32(shift) <= ecWindow-9 && len(src)-pos >= 8 {
		u := binary.BigEndian.Uint64(src[pos:])
		// XOR bytes 0..n-1 of u at bit positions shift-8k. Byte n straddles
		// bit 0 after the shift; mask its leaked top bits below shift&7 so the
		// operand matches the whole-byte loop exactly.
		dif ^= (u >> uint(56-shift)) &^ (1<<uint(shift&7) - 1)
		n := shift>>3 + 1
		pos += int(n)
		cnt += 8 * n
...
//go:noinline
//go:nosplit
func refillStateSlow(src []byte, pos int, dif uint64, cnt int32, tellOffs int32) (int, uint64, int32, int32) {
	shift := ecWindow - 9 - (cnt + 15)
	for shift >= 0 && pos < len(src) {
		dif ^= uint64(byteAtUnchecked(src, pos)) << uint(shift)
		cnt += 8
		shift -= 8
		pos++
	}
...
//go:nosplit
func (c *Cursor) readBitTrustedTrace() uint8 {
	dif := c.dif
...
func (r *Reader) ReadSymbol(cdf []uint16, symbols int) (int, error) {
	...
	rangeValue := uint32(r.rng)
	rngHi := rangeValue >> 8
	coded := uint32(r.dif >> (ecWindow - 16))
	upper := rangeValue
	lower := uint32(0)
	symbol := 0
	...
	dif := r.dif - uint64(lower)<<(ecWindow-16)
	rng := upper - lower
...
func (c *Cursor) ReadCDFUnchecked(cdf *CDF) int {
	...
	case 6:
		return c.readCDF6Known(&cdf.values)
	case 7:
		return c.readCDF7Known(&cdf.values)
	case 8:
		return c.readCDF8Known(&cdf.values)
	case 9:
		return c.readCDF9Known(&cdf.values)
	case 10:
		return c.readCDF10Known(&cdf.values)
	case 11:
		return c.readCDF11Known(&cdf.values)
	default:
...
//go:nosplit
func (c *Cursor) readCDF6Known(values *[MaxSymbols + 1]uint16) int {
	src := c.src
	pos := int(c.pos)
	dif := c.dif
	rng := uint32(c.rng)
	...
	c3 := uint32(values[3])
	c4 := uint32(values[4])
	lower := ((rngHi * (c0 >> ecProbShift)) >> (7 - ecProbShift)) + 5*ecMinProb
	symbol := 0
	if coded < lower {
		symbol = 1
		upper = lower
		lower = ((rngHi * (c1 >> ecProbShift)) >> (7 - ecProbShift)) + 4*ecMinProb
		if coded < lower {
			symbol = 2
			upper = lower
			lower = ((rngHi * (c2 >> ecProbShift)) >> (7 - ecProbShift)) + 3*ecMinProb
			if coded < lower {
				symbol = 3
				upper = lower
				lower = ((rngHi * (c3 >> ecProbShift)) >> (7 - ecProbShift)) + 2*ecMinProb
				if coded < lower {
					symbol = 4
					upper = lower
					lower = ((rngHi * (c4 >> ecProbShift)) >> (7 - ecProbShift)) + ecMinProb
					if coded < lower {
						symbol = 5
						upper = lower
						lower = 0
	...
	if traceEntropyReads {
		traceCDFRead(uint16(c0), 6, dif, rng, readerTell(pos, cnt, tellOffs))
	}
	dif -= uint64(lower) << (ecWindow - 16)
	rng = upper - lower
	shift := normShift16(rng)
...
//go:nosplit
func (c *Cursor) readCDF7Known(values *[MaxSymbols + 1]uint16) int {
	src := c.src
	pos := int(c.pos)
	dif := c.dif
	rng := uint32(c.rng)
	...
	c4 := uint32(values[4])
	c5 := uint32(values[5])
	lower := ((rngHi * (c0 >> ecProbShift)) >> (7 - ecProbShift)) + 6*ecMinProb
	symbol := 0
	if coded < lower {
		symbol = 1
		upper = lower
		lower = ((rngHi * (c1 >> ecProbShift)) >> (7 - ecProbShift)) + 5*ecMinProb
		if coded < lower {
			symbol = 2
			upper = lower
			lower = ((rngHi * (c2 >> ecProbShift)) >> (7 - ecProbShift)) + 4*ecMinProb
			if coded < lower {
				symbol = 3
				upper = lower
				lower = ((rngHi * (c3 >> ecProbShift)) >> (7 - ecProbShift)) + 3*ecMinProb
				if coded < lower {
					symbol = 4
					upper = lower
					lower = ((rngHi * (c4 >> ecProbShift)) >> (7 - ecProbShift)) + 2*ecMinProb
					if coded < lower {
						symbol = 5
						upper = lower
						lower = ((rngHi * (c5 >> ecProbShift)) >> (7 - ecProbShift)) + ecMinProb
						if coded < lower {
							symbol = 6
							upper = lower
							lower = 0
	...
	if traceEntropyReads {
		traceCDFRead(uint16(c0), 7, dif, rng, readerTell(pos, cnt, tellOffs))
	}
	dif -= uint64(lower) << (ecWindow - 16)
	rng = upper - lower
	shift := normShift16(rng)
...
//go:nosplit
func (c *Cursor) readCDF9Known(values *[MaxSymbols + 1]uint16) int {
	src := c.src
	pos := int(c.pos)
	dif := c.dif
	rng := uint32(c.rng)
	...
	c6 := uint32(values[6])
	c7 := uint32(values[7])
	lower := ((rngHi * (c0 >> ecProbShift)) >> (7 - ecProbShift)) + 8*ecMinProb
	symbol := 0
	if coded < lower {
		symbol = 1
		upper = lower
		lower = ((rngHi * (c1 >> ecProbShift)) >> (7 - ecProbShift)) + 7*ecMinProb
		if coded < lower {
			symbol = 2
			upper = lower
			lower = ((rngHi * (c2 >> ecProbShift)) >> (7 - ecProbShift)) + 6*ecMinProb
			if coded < lower {
				symbol = 3
				upper = lower
				lower = ((rngHi * (c3 >> ecProbShift)) >> (7 - ecProbShift)) + 5*ecMinProb
				if coded < lower {
					symbol = 4
					upper = lower
					lower = ((rngHi * (c4 >> ecProbShift)) >> (7 - ecProbShift)) + 4*ecMinProb
					if coded < lower {
						symbol = 5
						upper = lower
						lower = ((rngHi * (c5 >> ecProbShift)) >> (7 - ecProbShift)) + 3*ecMinProb
						if coded < lower {
							symbol = 6
							upper = lower
							lower = ((rngHi * (c6 >> ecProbShift)) >> (7 - ecProbShift)) + 2*ecMinProb
							if coded < lower {
								symbol = 7
								upper = lower
								lower = ((rngHi * (c7 >> ecProbShift)) >> (7 - ecProbShift)) + ecMinProb
								if coded < lower {
									symbol = 8
									upper = lower
									lower = 0
	...
	if traceEntropyReads {
		traceCDFRead(uint16(c0), 9, dif, rng, readerTell(pos, cnt, tellOffs))
	}
	dif -= uint64(lower) << (ecWindow - 16)
	rng = upper - lower
	shift := normShift16(rng)
...
//go:nosplit
func (c *Cursor) readCDF10Known(values *[MaxSymbols + 1]uint16) int {
	src := c.src
	pos := int(c.pos)
	dif := c.dif
	rng := uint32(c.rng)
	...
	c7 := uint32(values[7])
	c8 := uint32(values[8])
	lower := ((rngHi * (c0 >> ecProbShift)) >> (7 - ecProbShift)) + 9*ecMinProb
	symbol := 0
	if coded < lower {
		symbol = 1
		upper = lower
		lower = ((rngHi * (c1 >> ecProbShift)) >> (7 - ecProbShift)) + 8*ecMinProb
		if coded < lower {
			symbol = 2
			upper = lower
			lower = ((rngHi * (c2 >> ecProbShift)) >> (7 - ecProbShift)) + 7*ecMinProb
			if coded < lower {
				symbol = 3
				upper = lower
				lower = ((rngHi * (c3 >> ecProbShift)) >> (7 - ecProbShift)) + 6*ecMinProb
				if coded < lower {
					symbol = 4
					upper = lower
					lower = ((rngHi * (c4 >> ecProbShift)) >> (7 - ecProbShift)) + 5*ecMinProb
					if coded < lower {
						symbol = 5
						upper = lower
						lower = ((rngHi * (c5 >> ecProbShift)) >> (7 - ecProbShift)) + 4*ecMinProb
						if coded < lower {
							symbol = 6
							upper = lower
							lower = ((rngHi * (c6 >> ecProbShift)) >> (7 - ecProbShift)) + 3*ecMinProb
							if coded < lower {
								symbol = 7
								upper = lower
								lower = ((rngHi * (c7 >> ecProbShift)) >> (7 - ecProbShift)) + 2*ecMinProb
								if coded < lower {
									symbol = 8
									upper = lower
									lower = ((rngHi * (c8 >> ecProbShift)) >> (7 - ecProbShift)) + ecMinProb
									if coded < lower {
										symbol = 9
										upper = lower
										lower = 0
	...
	if traceEntropyReads {
		traceCDFRead(uint16(c0), 10, dif, rng, readerTell(pos, cnt, tellOffs))
	}
	dif -= uint64(lower) << (ecWindow - 16)
	rng = upper - lower
	shift := normShift16(rng)
...
//go:nosplit
func (c *Cursor) readCDF11Known(values *[MaxSymbols + 1]uint16) int {
	src := c.src
	pos := int(c.pos)
	dif := c.dif
	rng := uint32(c.rng)
	...
	c8 := uint32(values[8])
	c9 := uint32(values[9])
	lower := ((rngHi * (c0 >> ecProbShift)) >> (7 - ecProbShift)) + 10*ecMinProb
	symbol := 0
	if coded < lower {
		symbol = 1
		upper = lower
		lower = ((rngHi * (c1 >> ecProbShift)) >> (7 - ecProbShift)) + 9*ecMinProb
		if coded < lower {
			symbol = 2
			upper = lower
			lower = ((rngHi * (c2 >> ecProbShift)) >> (7 - ecProbShift)) + 8*ecMinProb
			if coded < lower {
				symbol = 3
				upper = lower
				lower = ((rngHi * (c3 >> ecProbShift)) >> (7 - ecProbShift)) + 7*ecMinProb
				if coded < lower {
					symbol = 4
					upper = lower
					lower = ((rngHi * (c4 >> ecProbShift)) >> (7 - ecProbShift)) + 6*ecMinProb
					if coded < lower {
						symbol = 5
						upper = lower
						lower = ((rngHi * (c5 >> ecProbShift)) >> (7 - ecProbShift)) + 5*ecMinProb
						if coded < lower {
							symbol = 6
							upper = lower
							lower = ((rngHi * (c6 >> ecProbShift)) >> (7 - ecProbShift)) + 4*ecMinProb
							if coded < lower {
								symbol = 7
								upper = lower
								lower = ((rngHi * (c7 >> ecProbShift)) >> (7 - ecProbShift)) + 3*ecMinProb
								if coded < lower {
									symbol = 8
									upper = lower
									lower = ((rngHi * (c8 >> ecProbShift)) >> (7 - ecProbShift)) + 2*ecMinProb
									if coded < lower {
										symbol = 9
										upper = lower
										lower = ((rngHi * (c9 >> ecProbShift)) >> (7 - ecProbShift)) + ecMinProb
										if coded < lower {
											symbol = 10
											upper = lower
											lower = 0
	...
	if traceEntropyReads {
		traceCDFRead(uint16(c0), 11, dif, rng, readerTell(pos, cnt, tellOffs))
	}
	dif -= uint64(lower) << (ecWindow - 16)
	rng = upper - lower
	shift := normShift16(rng)
