...
- `WebRTCEncoder` is the lower-level control/metadata surface for WebRTC
  picture scheduling. Its exported catalog follows pinned-libwebrtc
  (`L*T*`, `L*T*h`, `L*T*_KEY`, `L2T2_KEY_SHIFT`, and `S*T*`/`S*T*h`
  simulcast names), while explicit RTC configs also retain the implemented W3C
  key-shift schedules; it validates temporal/spatial layer structures,
  decode-target grids, dependency descriptors, active decode target masks,
  pinned-libwebrtc `L2T2_KEY_SHIFT` dependency templates, explicit
