...
buffering, loss timing policy, retransmission pacing, and RTX wrapping remain
with the caller. RTCP helpers cover generic packet and compound
...
stores bounded sender-side raw RTP history and appends packets requested by
Generic NACK pairs for caller-owned retransmission or RTX policy. The `RunLowOverheadInto`,
