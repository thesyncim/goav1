...
RTP header-extension mappings for dependency descriptors, transport/video
...
`EncoderWebRTCRTCPFeedbackRequiresKeyFrame` and
`EncoderWebRTCRTCPPacketsRequireKeyFrame` classify parsed single or compound
PLI/FIR/LRR feedback into the existing `forceKey` argument when a full refresh
...
- `VideoEncoder` / `RTCEncoder` is the friendly realtime pixel encoder. It
  ...
  caller-owned buffers; `AppendEncoderWebRTCRTPPacketsWithHeaders` wraps those
  bodies into complete RTP packets with fixed headers and an RFC 8285
  dependency-descriptor header extension; `RTCFrame.AppendRTPPacketsWithOptions`,
