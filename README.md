...
extension IDs; WebRTC RTP helpers also parse and write raw CVO, playout-delay,
...
| Tile groups | Single and multi-tile groups pass current strict-MD5 gates; tile-list OBUs parse, validate source reference anchors/uniform decode layout, resolve external anchor frames through provider-backed surface IDs, expose libaom-shaped output geometry/format/copy regions, plan raw tile-list entry decode jobs, bind each entry's external anchor as LAST_FRAME for residual decode, and provide entry-level plus whole-list external-reference-to-output blit helpers, but automatic tile-list playback/output publication is not wired yet |
