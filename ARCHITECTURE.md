...
| WebRTC signaling helpers              | Complete for AV1/90000 SDP/fmtp profile/level/tier checks, RTP header-extension mapping checks, AV1 RID receiver restrictions, AV1 simulcast RID groups, AV1 rtcp-fb checks, and AV1 RTCP Layer Refresh Request FCI parse/build/validation. |
...
- **Full WebRTC media transport.** The package emits AV1 RTP payload bodies and
  dependency descriptors, and exposes focused AV1 SDP/fmtp/extmap/RID/simulcast
  plus RTCP LRR helper surfaces. RTP headers, SRTP, full SDP assembly, jitter
  buffering, loss policy, pacing, and retransmission remain integration-layer
  responsibilities.
...
1. **Broaden the WebRTC realtime encoder.** Add high-bit-depth/non-4:2:0
   inputs, richer tuning controls, broader libaom/libwebrtc/SVT oracle
   coverage, and measured compression-efficiency tuning.
