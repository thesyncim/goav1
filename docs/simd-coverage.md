...
| Inter prediction/convolve | Convolve, compound, joint compound, scale, warp, highbd, DOTPROD and I8MM variants | 8-bit and highbd X/Y/2D convolve have NEON/AVX2; arm64 8-bit X/Y one-axis convolve now keeps width>=8 4-tap filters on the NEON body instead of falling back to scalar. Compound paths reuse convolve/blend, no dotprod/i8mm tier. | Baseline coverage is better for active 4-tap one-axis paths, but max-tier coverage is still not equivalent. DOTPROD/I8MM should come after CPU feature detection and profiler confirmation. |
