...
| Loop filter, CDEF, restoration, superres, film grain | Broad NEON and highbd variants | goav1 has NEON/AVX2 coverage for these postfilter/decoder-style kernels. Arm64 8-wide CDEF now has a dav1d-shaped primary-only split entry point; secondary-only and both-strength clipping still use the generic NEON filter. | Mostly covered for current profile. Loopfilter still appears, so optimize only with a focused profile. Continue CDEF split work one measured strength case at a time. |
...
- arm64 CDEF 8-wide primary-only blocks now route to a dav1d-shaped split NEON
  ...
  secondary-only remains on the generic NEON kernel until a clean measured split
  lands.
