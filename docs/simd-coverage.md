...
| TXB coefficient prep and contexts | `encodetxb_neon.c`: `svt_av1_txb_init_levels_neon`, `svt_av1_get_nz_map_contexts_neon`; `av1_quantize_neon.c`: `svt_av1_compute_cul_level_neon` | Public coefficient level initialization now has an arm64 NEON family mapped to `svt_av1_txb_init_levels_neon` for all adjusted TXB scan heights 4, 8, 16, and 32, covering square, rectangular, and adjusted 64-sized TXBs. Public Class2D NZ-map context extraction now has an arm64 NEON family mapped to SVT's five-neighbour `svt_av1_get_nz_map_contexts_neon` kernel for local column-major scan heights 4, 8, 16, and 32, covering 4x4 through adjusted 64-sized TXBs and rectangular 8x4/16x8/4x16/8x32-shaped scans while preserving the public partial-EOB context contract. The active trusted 8x8/Class2D count path also uses a dedicated arm64 NEON level-init helper, with scalar source-shaped scan summary for nonzero/sign bits, max scan line, and cumulative-level context. The active trusted 16x16 luma count path now reuses the 16-row Class2D NZ-map kernel for lower-context lookup. Hot Go writer has trusted 4x4/8x8/16x16/32x32 count-only trial paths, stack level buffers, fixed CDF storage, and recorded sign bits/nonzero bitsets on measured-winning count paths. | Level-init prep is covered for ARM64 baseline NEON. Class2D full-map context extraction is covered for scan heights 4, 8, 16, and 32, with the measured-winning 16x16 luma count path wired; horizontal/vertical class context kernels, broader inline writer context use, and `svt_av1_compute_cul_level_neon` remain open ARM64 parity work. |
