...
| Forward transforms | `highbd_fwd_txfm_neon.c` covers square, rectangular, N2/N4, and many tx types including ADST paths | Forward DCT 4/8/16/32 has NEON; the active trusted 8x8 IDTX, ADST_DCT, DCT_ADST, and ADST_ADST tx-type trials now have arm64 NEON matching SVT's identity, ADST-column/DCT-row, DCT-column/ADST-row, and ADST-column/ADST-row surfaces. The generic source-shaped forward path now covers the non-DCT_DCT FlipADST, ADST16, and 1D DCT/ADST/IDTX combinations whose axes fit the local 1D kernels; DCT64-dependent forward shapes remain unsupported until a real DCT64 port lands. | High for the current profile: the active 8x8 tx-type trial surface is covered, and broader scalar correctness is better, but SVT's wider SIMD matrix still includes rectangular/N2/N4/flip ADST and DCT64/N2/N4 variants. |
...
- `ForwardBlock` now routes non-DCT_DCT transform families that are not covered
  ...
  combinations backed by local 1-D kernels; DCT64-dependent forward transforms
  still reject instead of approximating. The new tests cover flip-shape parity,
  Q13 core parity against the existing 4/8-point kernels, inverse round-trip
  rows across square/rectangular FlipADST/ADST/1D cases, and explicit rejection
  for ADST32/DCT64-dependent shapes.
