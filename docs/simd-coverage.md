# SIMD Coverage Against SVT-AV1 and dav1d

This ledger tracks whether goav1 has the same useful assembly coverage as the
SVT-AV1 and dav1d builds used for speed comparisons. It is intentionally profile-driven:
matching every upstream file name is not the goal; matching the active kernels
that matter for this encoder/decoder, and not giving a reference implementation an unreported SIMD tier
...
- dav1d repository pin: `third_party/upstream/dav1d`, tag `1.5.3`, commit
  `b546257f770768b2c88258c533da38b91a06f737`
- dav1d reference assembly families: arm64 `cdef`, `filmgrain`, `ipred`,
  `itx`, `loopfilter`, `looprestoration`, `mc`, `mc_dotprod`, `msac`,
  `refmvs`; x86 SSE/AVX2/AVX512 equivalents under `src/x86`.
...
remaining TXB prep/context extraction, forward ADST/hybrid transform trials,
and any search/RD metrics still proven hot after the existing SAD and
residual-stat NEON kernels. DOTPROD/I8MM convolve and CDEF are max-tier
fairness gaps, but they should follow a profile row that shows those paths are
active enough to matter in the measured encoder mode.

For dav1d-class decode parity, the broad source families to keep mapped are
motion compensation (`mc`, `mc_dotprod`, `mc16`), inverse transforms (`itx`,
`itx16`), intra prediction (`ipred`, `ipred16`), CDEF, loop filter,
restoration, film grain, entropy/range coding (`msac`), and reference-MV
projection (`refmvs`). goav1 has SIMD in several DSP/postfilter families, but
...
| --- | --- | --- | --- |
| CPU feature tiers | `ASM_NEON`, `ASM_NEON_DOTPROD`, `ASM_NEON_I8MM`, `ASM_SVE`, `ASM_SVE2` | arm64 detects NEON plus Darwin DOTPROD/I8MM/SVE/SVE2 feature bits. Metric dispatch selects a measured DOTPROD tier for winning pixel SSE/variance rows, SAD/search dispatch selects DOTPROD for measured-winning step-4 x4 raster groups, lowbd resident single-prediction X/Y dispatch and 2D caller-scratch dispatch select I8MM, including width-multiple-of-8 blocks and the width-4 zero-ended 4-tap 2D path. Lowbd resident compound-X CONV_BUF dispatch selects DOTPROD on Apple M4. The same compound-X surface now has a direct-tested I8MM tier mapped to SVT's `dist_wtd_convolve_x_8tap_neon_i8mm`; it is left behind DOTPROD when both features are present because DOTPROD wins the local benchmark. Lowbd compound 2D also has a direct-tested width-4 4-tap I8MM tier. The rest of the active arm64 dispatch remains baseline NEON unless another feature-tier kernel is proved. | Do not claim max-tier parity until broader DOTPROD/I8MM convolve, broader search, CDEF, and remaining metric surfaces are wired and measured. Pin SVT with `-svt-asm neon` for baseline-tier rows. |
| TXB coefficient prep and contexts | `encodetxb_neon.c`: `svt_av1_txb_init_levels_neon`, `svt_av1_get_nz_map_contexts_neon`; `av1_quantize_neon.c`: `svt_av1_compute_cul_level_neon` | Public coefficient level initialization now has an arm64 NEON family mapped to `svt_av1_txb_init_levels_neon` for all adjusted TXB scan heights 4, 8, 16, and 32, covering square, rectangular, and adjusted 64-sized TXBs. Public Class2D, horizontal, and vertical NZ-map context extraction now have arm64 NEON families mapped to SVT's five-neighbour `svt_av1_get_nz_map_contexts_neon` kernels for local column-major scan heights 4, 8, 16, and 32, covering 4x4 through adjusted 64-sized TXBs and rectangular 8x4/16x8/4x16/8x32-shaped scans while preserving the public partial-EOB context contract and the Class2D-only DC zero rule. A direct arm64 NEON cumulative-level reducer now maps SVT's standalone `svt_av1_compute_cul_level_neon` shape for the local int16 coefficient ABI and is parity-tested against scalar. The production dispatch remains scalar because the local scalar reducer can early-saturate and the active writer paths still need absolute levels, signs, nonzero masks, and max-scan state. The active trusted 8x8/Class2D count path also uses a dedicated arm64 NEON level-init helper, with scalar source-shaped scan summary for nonzero/sign bits, max scan line, and cumulative-level context. The active trusted 16x16 luma count path now reuses the 16-row Class2D NZ-map kernel for lower-context lookup. Hot Go writer has trusted 4x4/8x8/16x16/32x32 count-only trial paths, stack level buffers, fixed CDF storage, and recorded sign bits/nonzero bitsets on measured-winning count paths. | Level-init prep, full-map Class2D/horizontal/vertical context extraction, and direct standalone cumulative-level parity coverage are covered for ARM64 baseline NEON. Broader inline writer context use remains open; do not wire the standalone cumulative-level pass into production until an active writer benchmark improves. |
| Range coder and CDF update | SVT does not make this a comparable named SIMD surface; arithmetic coding is serial; dav1d keeps the bool/range reader in its `msac` assembly family | Cursor equiprobable bit decode now has an arm64 assembly helper for the non-trace production build, with the source-shaped Go body retained for `purego`, non-arm64, and `goav1_trace_rng`. `WriteBinaryCDFTrusted`, `WriteCDF4`/`WriteCDF5`/`WriteCDF7`, `normalize`, and `WriteBit` remain top scalar cleanup entries. Fixed-arity writer/counter streams gate the exact count-only paths. | The read-side bool helper is covered for arm64 because it beats the generic path in direct, CDF, and full-vector gates while staying zero-allocation. Keep the remaining range writer/CDF paths source-shaped Go unless a benchmark proves assembly beats call/setup cost. |
...
| Inter prediction/convolve | SVT convolve, compound, joint compound, scale, warp, highbd, DOTPROD and I8MM variants; dav1d `mc`, `mc_dotprod`, and `mc16` | 8-bit and highbd X/Y/2D convolve have NEON/AVX2; arm64 8-bit single-prediction X, Y, and caller-scratch 2D now have live I8MM tiers for resident width-multiple-of-8 blocks plus width-4 zero-ended 4-tap 2D blocks. X and width-multiple-of-8 2D cover zero-ended 4-tap filters through the source-shaped 8-tap body; width-4 2D uses a dedicated SVT-shaped USDOT horizontal pass into the local int16 intermediate. Y has separate source-shaped 4-tap and 8-tap vertical USDOT paths matching SVT's `convolve_y_sr_4tap_neon_i8mm` and `convolve_y_sr_8tap_neon_i8mm`, including width-4 rows with exact four-byte stores. The width-multiple-of-8 2D I8MM path maps SVT's `convolve_2d_sr_horiz_8tap_neon_i8mm` horizontal pass into the local int16 intermediate, then reuses the proven NEON vertical reducer. The 8-bit compound/joint-convolve copy, X-only, Y-only, and filtered 2D branches have arm64 NEON kernels for resident width-multiple-of-8 and resident width-4 CONV_BUF blocks, with a measured DOTPROD tier for resident width-multiple-of-8 X-only. That resident compound X-only surface also has a direct-tested I8MM `usmmla` tier for width-multiple-of-8 blocks; DOTPROD stays first on Apple M4 because it is faster. Lowbd compound filtered 2D now adds a direct-tested width-4 4-tap I8MM USDOT path for the common caller-scratch CONV_BUF block shape. High-bit-depth compound copy, X-only, Y-only, and 2D CONV_BUF resident paths now have live arm64 NEON coverage for 10-bit and 12-bit. Identity-step scaled compound now routes into those same ARM compound kernels for 8/10/12-bit; non-identity scaled compound and the remaining DOTPROD/I8MM compound variants remain uncovered. 8-bit clamped X/Y/2D edge blocks now split exact pure-Go clamped strips from resident SIMD rectangles on arm64 NEON and amd64 AVX2, including caller-scratch dispatch for 2D. | Baseline edge-convolve coverage is now materially better for the active WebRTC encoder profile and closer across arm64/amd64. Max-tier coverage is still not equivalent. Continue DOTPROD/I8MM convolve only with source-shaped ports and paired CPU-feature benchmark proof. |
...
| WebRTC spatial scaling | SVT has superres-style resize helpers; WebRTC SVC preparation needs deterministic layer downscale before encode | Exact nearest-neighbor WebRTC spatial-layer scaling has arm64 NEON row kernels for 8-bit and 10/12-bit planes on exact 2:1 and 4:1 downscale. Other ratios, tails, and non-arm64 builds preserve the scalar reference path. | Covered for common L2/L3 realtime WebRTC layer downscale on arm64. Future ratios should be added only with a source-shaped reference and WebRTC-facing quality tests. |
| Loop filter, CDEF, restoration, superres, film grain | Broad NEON and highbd variants | goav1 has NEON/AVX2 coverage for these postfilter/decoder-style kernels. Arm64 8-wide and width-4 CDEF now have dav1d-shaped primary-only and secondary-only split entry points; both-strength clipping still uses the generic NEON filter. | Mostly covered for current profile. Loopfilter still appears, so optimize only with a focused profile. Continue CDEF split work one measured strength case at a time. |
...
- CDF state is fixed `uint16` storage, not oversized heap maps or slices in the
  hot writer path.
- arm64 non-trace cursor equiprobable bit decode now uses a package-level
  assembly helper with source-shaped Go fallbacks for `purego`, non-arm64, and
  `goav1_trace_rng`. On Apple M4 Max, direct paired medians were
  `BenchmarkCursorReadBitTrustedStream` arm64 `15093 ns/op` versus pure-Go
  `16672 ns/op`, `BenchmarkCursorCDF4Stream` arm64 `22118 ns/op` versus
  pure-Go `22683 ns/op`, and `BenchmarkCursorCDF4HighTokenStream` arm64
  `24569 ns/op` versus pure-Go `24848 ns/op`, all zero allocations. The
  same-session full-vector decode median was arm64 `18854674 ns/op` versus
  pure-Go `19004571 ns/op`, also zero-allocation, so this is a modest
  connected decoder win rather than only a microbench result.
...
- arm64 CDEF 8-wide primary-only blocks now route to a dav1d-shaped split NEON
  entry point. On Apple M4 Max,
  `BenchmarkFilterBlock8x8PrimaryOnlyDispatchNEON` reports `45.38-46.03 ns/op`
  versus a generic-dispatch comparison at `46.67-46.86 ns/op`, all zero
  allocations. Direct generic and split asm both measure around
  `43.1-43.9 ns/op`, so the win is only in the dispatch-shaped path;
  secondary-only now has the same split-dispatch treatment.
- arm64 CDEF 8-wide secondary-only blocks now route to a dav1d-shaped split
  NEON entry point. The previous live dispatch baseline was
  `77.12-77.79 ns/op`; the split dispatch reports `73.34-73.97 ns/op`, while
  direct generic and split bodies tie at roughly `70.6-71.2 ns/op`, all zero
  allocations. This closes the secondary-only split coverage gap without
  claiming a faster direct math body; both-strength clipping remains on the
  generic NEON kernel.
- arm64 CDEF width-4 primary-only and secondary-only blocks now have matching
  split NEON entry points for 4:2:0 chroma shapes. On Apple M4 Max,
  `BenchmarkFilterBlock4x4PrimaryOnlyDispatchNEON` reports
  `29.46-30.07 ns/op`, and `BenchmarkFilterBlock4x4SecondaryOnlyDispatchNEON`
  reports `37.50-40.21 ns/op`, all zero allocations. Direct generic and split
  bodies tie (`22.80-23.61 ns/op` for primary-only and `35.03-35.58 ns/op` for
  secondary-only), so this is also a source-shaped dispatch/coverage split, not
  a claim that the direct lane math became faster. Both-strength clipping
  remains on the generic NEON kernel.
- arm64 8-bit one-axis convolve now routes width>=8 4-tap X/Y filters through
  the existing NEON 8-tap body; zero end taps are bit-exact and much faster
  than the previous scalar fallback. On Apple M4 Max, `BenchmarkConvolveX8_4tap_32`
  moved from a `1185 ns/op` median to `275.2 ns/op`, and
  `BenchmarkConvolveY8_4tap_32` moved from `1171 ns/op` to `247.4 ns/op`, all
  zero allocations.
- arm64 8-bit single-prediction X convolve now has an I8MM tier matching SVT's
  `svt_av1_convolve_x_sr_neon_i8mm` 8-tap output branch: it permutes with
  `svt_kMatMul8PermuteTbl`, multiplies f1..f7 through `usmmla`, subtracts tap
  0 separately, adds the single-prediction horizontal round shim, and finishes
  with signed saturating rounded narrow to 8-bit pixels. On Apple M4 Max,
  `BenchmarkConvolveX8_32` dispatch reports `98.01-99.83 ns/op`, direct I8MM
...
- arm64 TXB NZ-map context extraction now covers Class2D, horizontal, and
  vertical classes with source-shaped NEON kernels for scan heights 4, 8, 16,
  and 32. On Apple M4 Max, `BenchmarkCoeffNZMapContexts` reports zero
  allocations and representative dispatch/scalar pairs of `4x4/Class2D`
  `20.69 ns/op` vs `71.91 ns/op`, `4x4/horiz` `19.89 ns/op` vs
  `75.15 ns/op`, `8x8/vert` `37.29 ns/op` vs `309.7 ns/op`,
  `16x16/horiz` `128.8 ns/op` vs `1175 ns/op`, and `32x32/vert`
  `376.5 ns/op` vs `5015 ns/op`.
...
- arm64 WebRTC exact nearest-neighbor spatial downscale has NEON row kernels for
  8-bit and 10/12-bit planes. On Apple M4 Max, the 8-bit 1080p scaler moved
  from roughly `271 us/op` to `18.3 us/op` for 2:1 and from `68.3 us/op` to
  `9.0 us/op` for 4:1. The 16-bit row follows the same deinterleave shape with
  `ld2.8h`/`ld4.8h`, moving from roughly `272 us/op` to `32.8 us/op` for 2:1
  and from `68.8 us/op` to `16.4 us/op` for 4:1. Tails and unsupported ratios
  remain scalar, and all rows are zero-allocation.
- arm64 8-bit single-prediction 2D convolve now has an I8MM tier matching the
  ...
  baseline NEON 2D path, then reuses the proven vertical reducer. On Apple M4
  Max, the no-scratch dispatch stays on NEON at `1094-1108 ns/op`, while the
  live caller-scratch I8MM dispatch reports `328.2-335.3 ns/op` and nil-scratch
  I8MM through the same dispatch slot reports `945.5-983.5 ns/op`; direct NEON
  scratch reports `520.7-610.5 ns/op`, all zero allocations. Width>=8
  zero-ended 4-tap filters also ride the I8MM body, measuring
  `942.4-962.8 ns/op` in the focused scratchless direct row.
...
- arm64 8-bit compound X-to-CONV_BUF now has a DOTPROD tier matching SVT's
  `dist_wtd_convolve_x_neon_dotprod` / `svt_av1_jnt_convolve_x_neon_dotprod`
  do_average==0 branch: subtract 128 from samples, halve even taps, fold the
  range correction plus CONV_BUF round offset into the accumulator, then SDOT
  and shift by `ROUND0_BITS-1`. dav1d's related arm64 signal is
  `src/arm/64/mc_dotprod.S`, which promotes 8-tap MC/prep to `neon_dotprod`
  and `neon_i8mm`. On Apple M4 Max, `BenchmarkCompoundConvBufX8_32` dispatch
  reports `96.14-96.56 ns/op`, direct DOTPROD reports `89.52-89.80 ns/op`,
  and direct NEON reports `257.8-260.2 ns/op`, all zero allocations.
- arm64 8-bit compound X-to-CONV_BUF also has an I8MM tier matching SVT's
  `dist_wtd_convolve_x_8tap_neon_i8mm` / `svt_av1_jnt_convolve_x_neon_i8mm`
  no-average branch: permute with `svt_kMatMul8PermuteTbl`, multiply f1..f7
  through `usmmla`, subtract tap 0 separately, add the CONV_BUF round shim, and
  shift by `ROUND0_BITS-1`. The direct I8MM row is exact and zero-allocation,
  but Apple M4 keeps live dispatch on DOTPROD: recent paired rows were dispatch
  `93.87-94.48 ns/op`, direct DOTPROD `87.50-88.23 ns/op`, direct I8MM
  `102.9-103.9 ns/op`, and direct NEON `251.7-253.3 ns/op`.
- arm64 high-bit-depth compound X-to-CONV_BUF now mirrors SVT's no-average
  `svt_av1_highbd_jnt_convolve_x_neon` branch for resident blocks while keeping
  goav1's 12-bit `round0` adjustment explicit. Width-4 uses the source-shaped
  4-tap `x_filter_ptr+2` path; width>=8 uses the 8-tap body and is covered for
  10-bit and 12-bit. On Apple M4 Max, `BenchmarkCompoundConvBufXHighBD_32`
  live dispatch reports `215.8-228.0 ns/op`, direct NEON reports
  `202.8-204.5 ns/op`, and direct scalar reports `3643-3692 ns/op`, all zero
  allocations.
- arm64 high-bit-depth compound Y-to-CONV_BUF now mirrors SVT's no-average
  `svt_av1_highbd_jnt_convolve_y_neon` branch for resident blocks while keeping
  goav1's 12-bit `round0` adjustment explicit. Width-4 uses exact four-sample
  row loads and stores; width>=8 uses 8-sample vertical tap rows and is covered
  for 10-bit and 12-bit. On Apple M4 Max,
  `BenchmarkCompoundConvBufYHighBD_32` live dispatch reports
  `209.6-211.6 ns/op`, direct NEON reports `200.6-202.8 ns/op`, and direct
  scalar reports `2748-2782 ns/op`, all zero allocations.
- arm64 high-bit-depth compound 2D-to-CONV_BUF now uses a resident NEON
  two-pass kernel for 10-bit and 12-bit: horizontal int32 intermediate
  `round(xBias + horizontal_sum, round0)`, then vertical CONV_BUF output
  `round(yBias + vertical_sum, COMPOUND_ROUND1_BITS)`. The wrapper falls back
  to the exact pure-Go resident path when the extra right-edge slide sample is
  not available. On Apple M4 Max, `BenchmarkCompoundConvBuf2DHighBD_32` live
  dispatch reports `920.4-929.2 ns/op`, direct NEON reports
  `888.9-899.7 ns/op`, and direct scalar reports `5957-6008 ns/op`, all zero
  allocations.
- Identity-step scaled compound now bypasses the generic scaled loop and routes
  through the unscaled compound CONV_BUF dispatch with the same subpel kernels,
  matching libaom/SVT's scaled math when `x_step == y_step == SCALE_SUBPEL_SCALE`.
  The scaled scratch now owns compound identity scratch for high-bit-depth 2D so
  the fast path stays zero-allocation. On Apple M4 Max,
  `BenchmarkScaledCompoundConvBufIdentity8_32` reports `1131-1144 ns/op`
  versus direct generic `17281-17353 ns/op`, and
  `BenchmarkScaledCompoundConvBufIdentityHighBD_32` reports
  `926.1-936.2 ns/op` versus direct generic `21537-21706 ns/op`, all zero
  allocations.
...
- arm64 8-bit compound 2D-to-CONV_BUF now mirrors SVT's no-average
  ...
  allocations.
- arm64/amd64 8-bit clamped X/Y/2D edge convolve now routes resident interior
  rectangles through the existing NEON/AVX2 kernels while keeping exact
  pure-Go clamping for edge strips. On Apple M4 Max,
  `BenchmarkConvolve2D8ClampedEdge_16WithScratch` measures about
  `1.46 us/op`, zero allocations. In the 1080p single-thread encoder profile,
  `convolve2D8ClampedPureGoWithIM` dropped from the previous `~0.31s` flat
  hotspot to `~0.06s`, and
  `BenchmarkVideoEncoderPFramePan1080pSingleThread` moved from the previous
  `~49.0 ms/op` range to `~46.5-47.6 ms/op`, still `0 B/op` and
  `0 allocs/op`.
  The Darwin amd64 test binary also runs under Rosetta on this machine and
  proves AVX2 X/Y/2D edge-split parity; its `VirtualApple` benchmark rows were
  about `1.08 us/op` for X and `2.42 us/op` for Y, both zero-allocation. Treat
  those rows as amd64 dispatch evidence, not native x86 throughput.
- arm64 8-bit width-4 compound CONV_BUF now has dedicated NEON rows for copy,
  X, Y, and 2D resident blocks. Paired pure-Go vs NEON medians on
  `BenchmarkCompoundConvBuf*_4x16` were: copy `56.11 ns/op` to `28.68 ns/op`,
  X `80.75 ns/op` to `32.08 ns/op`, Y `146.7 ns/op` to `128.5 ns/op`, and 2D
  `231.5 ns/op` to `59.39 ns/op`, all zero allocations.
...
- The trusted 8x8/Class2D coefficient count path now uses an arm64 NEON
  level-initialization helper mapped to SVT's `svt_av1_txb_init_levels_neon`.
  It writes the padded 8x8 level window with `min(abs(coeff), 127)` semantics;
  the exact scan-order nonzero/sign masks, max scan line, absolute levels, and
  cumulative-level context stay source-shaped in Go. On Apple M4 Max,
  `BenchmarkTXBPrep8x8Levels2D` measured NEON `95.13-95.25 ns/op` versus
  scalar `148.6-149.8 ns/op`, all zero allocations.
- Public TXB coefficient level initialization now has arm64 NEON kernels for
  adjusted scan heights 4, 8, 16, and 32, matching
  `svt_av1_txb_init_levels_neon`'s saturating `min(abs(coeff), 127)` behavior
  while preserving goav1's column-major scratch ABI. `BenchmarkCoeffInitLevels`
  on Apple M4 Max measured representative zero-allocation impl/scalar rows:
  4x4 `9.87-9.98 ns/op` vs `29.00-29.32 ns/op`, 8x8
  `12.09-12.33 ns/op` vs `83.68-84.88 ns/op`, 16x16
  `17.54-17.71 ns/op` vs `275.1-277.2 ns/op`, and adjusted 32x32/64x64
  `46.23-47.01 ns/op` vs `927.0-945.9 ns/op`.
- Public Class2D TXB NZ-map context extraction now has arm64 NEON kernels for
  adjusted scan heights 4, 8, 16, and 32, matching SVT's
  `svt_av1_get_nz_map_contexts_neon` five-neighbour
  `min((sum(min(level, 3))+1)>>1, 4)+offset` calculation while preserving
  goav1's column-major scratch ABI and partial-EOB public contract. On Apple
  M4 Max, `BenchmarkCoeffNZMapContexts` measured representative
  zero-allocation impl/scalar rows: 4x4 `20.00 ns/op` vs `71.22 ns/op`,
  8x8 `39.04 ns/op` vs `297.2 ns/op`, 8x4 `27.43 ns/op` vs
  `148.0 ns/op`, and 16x8 `71.99 ns/op` vs `579.7 ns/op`.
...
- `ForwardBlock` now routes non-DCT_DCT transform families that are not covered
  by the trusted 8x8 fast path through a source-shaped `fwd_txfm2d_c` port:
  column up/down flip, first shift, 1-D DCT/ADST/IDTX, middle shift,
  left/right buffer flip, row 1-D transform, final shift, factor-two
  rectangular sqrt2 scale, and transposed coefficient store. The scalar port
  adds FlipADST, ADST16, 16/32-point identity, and valid rectangular/1D
  combinations backed by local 1-D kernels. The DCT64 scalar path now ports
  libaom's `av1_fdct64`, adds Q10/Q11 forward cosine rows, and exposes the
  realtime `av1_fwd_txfm2d_64x64_c`, `32x64`, and `64x32` reduced-coefficient
  layouts without approximating the high-frequency truncation/repack step. The
  new tests cover flip-shape parity, Q13 core parity against the existing
  4/8-point kernels, pinned DCT64 vectors generated from the upstream C
  kernel, DC scale and reduced-plane dispatch for 64-extent DCT_DCT shapes,
  inverse round-trip rows across square/rectangular FlipADST/ADST/1D cases, and
  explicit rejection for ADST32 and non-realtime 16x64/64x16 forward shapes.
...
- A direct arm64 NEON cumulative-level reducer now maps SVT's standalone
  `svt_av1_compute_cul_level_neon` shape, including scan-order gathers and
  vector absolute-level accumulation, but it remains a parity surface instead
  of the production dispatch. The focused `BenchmarkCoeffCulLevel` reports
  zero allocations for scalar dispatch and direct arch rows; on the saturated
  fixture, scalar dispatch wins decisively because it stops after reaching
  `CoeffContextMask` while the standalone NEON reducer pays scan-gather and
  call overhead for the full EOB. Keep cumulative-level SIMD folded into a
  wider writer/prep kernel before claiming an encoder hot-path win.
...
2. Prototype a tiny TXB prep kernel only if it replaces work already proven hot:
   eob/level-buffer/stat extraction for the 8x8 luma path. A standalone
   cumulative-level-only pass is already parity-tested and rejected for
   dispatch; keep a wider split only if both
