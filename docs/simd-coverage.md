...
| Inter prediction/convolve | Convolve, compound, joint compound, scale, warp, highbd, DOTPROD and I8MM variants | 8-bit and highbd X/Y/2D convolve have NEON/AVX2; arm64 8-bit X/Y one-axis convolve now keeps width>=8 4-tap filters on the NEON body instead of falling back to scalar. The 8-bit compound/joint-convolve copy and X-only branches now have arm64 NEON kernels for resident width-multiple-of-8 CONV_BUF blocks. Filtered compound Y/2D paths remain scalar, and there is no dotprod/i8mm compound tier. | Baseline coverage is better for active 4-tap one-axis paths and the copy/X compound branches, but max-tier coverage is still not equivalent. DOTPROD/I8MM should come after CPU feature detection and profiler confirmation. |
...
- arm64 8-bit compound X-to-CONV_BUF now mirrors SVT's no-average
  ...
  Filtered compound Y/2D rows are unchanged and remain the larger convolve
  parity gap.
