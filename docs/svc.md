...
cover source-description chunks such as CNAME and TOOL; feedback packet helpers
(`ParseRTCPFeedbackPacket`) cover complete RTPFB/PSFB packet wrapping and
parsing; RTCP Transport-CC helpers (`ParseRTCPTransportFeedbackFCI`) cover
...
| WebRTC AV1 RTCP SR/RR/SDES/Transport-CC/REMB/LRR helpers | Complete for sender/receiver report packet parse/build, source-description/CNAME packet parse/build, RTPFB/PSFB packet wrapping/parsing, transport-wide status chunks, delta ticks, no-timestamp feedback, legacy WebRTC REMB bitrate/SSRC FCI parse/build, AV1 layer-index and LRR FCI entry/list parse/build/validation, plus encoder-config temporal/spatial layer-grid checks. |
