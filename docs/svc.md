...
| WebRTC AV1 SVC control metadata                     | Exported catalogs and active-encoding validation follow the pinned-libwebrtc mode catalog (`L*T*`, `L*T*h`, `L*T*_KEY`, `L2T2_KEY_SHIFT`, `S*T*`, `S*T*h`); explicit RTC configs also retain the implemented W3C key-shift temporal schedules, dependency-descriptor decode targets over the full `(spatial, temporal)` grid, and pinned-libwebrtc `L2T2_KEY_SHIFT` dependency templates. |
