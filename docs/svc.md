...
and signed 24-bit cumulative-loss fields; feedback packet helpers
...
| WebRTC AV1 RTCP SR/RR/Transport-CC/REMB/LRR helpers | Complete for sender/receiver report packet parse/build, RTPFB/PSFB packet wrapping/parsing, transport-wide status chunks, delta ticks, no-timestamp feedback, legacy WebRTC REMB bitrate/SSRC FCI parse/build, AV1 layer-index and LRR FCI entry/list parse/build/validation, plus encoder-config temporal/spatial layer-grid checks. |
