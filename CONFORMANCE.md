...
- **Encoder.** The friendly realtime encoder emits AV1 bitstreams for 8-bit
  ...
  simulcast RID groups, active-encoding scalabilityMode validation for the
  WebRTC-SVC single-SSRC simulcast rule, generic and compound RTCP packet
  parsing, RTCP SR/RR/SDES/BYE and RTPFB/PSFB packet wrapping/parsing,
...
- `make webrtc-production`: strict realtime/WebRTC gate requiring `aomdec` and
  ...
  RFC 4588 RTX wrap/parse/restore plus all-mode RTX repair through
...
- `make dryrun-corpus`: optional generated real-content corpus stream-MD5
  ...
  4:2:0/4:2:2, inter/intra, and tiled cases. The generator expects 25 clips,
  pins aomenc thread/row-mt settings, and writes a local ignored
  `manifest.tsv` with source, tool, IVF, and MD5 hashes. This target requires
  the local ignored corpus generated by `scripts/gen_bench_corpus.sh` and skips
