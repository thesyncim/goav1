...
- `make dryrun-corpus`: optional generated real-content corpus stream-MD5
  conformance. The current generated matrix is 23 clips / 1104 visible frames
  across 144p, 288p, 360p, 720p, 8/10/12-bit, 4:2:0/4:2:2,
  inter/intra, and tiled cases. This target requires the local ignored corpus
  generated by
  `scripts/gen_bench_corpus.sh` and skips cleanly when it is absent.
- `make dryrun-external-corpus`: optional local third-party corpus stream-MD5
  conformance. It recursively scans `GOAV1_EXTERNAL_CORPUS_DIR` or the path-list
  `GOAV1_EXTERNAL_CORPUS_DIRS` for `.ivf` files with sibling `.md5` stream
  digests, so Argon Streams, dav1d-test-data, FATE, or lab corpora can be
  mounted locally without vendoring their binaries into this repository.
